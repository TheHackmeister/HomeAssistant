"""Ensure the ``homeassistant`` stub is installed before pytest imports the
``labeled_features`` package during test collection.

pytest's conftest resolver walks up from each collected test file toward
``rootdir``, executing every ``conftest.py`` it finds *after* it has already
imported the parent packages of the test module (so that fixtures on those
packages are available). That order means the ``labeled_features`` package
gets imported before ``tests/conftest.py`` has a chance to install the
``homeassistant`` stub. Installing the stub from this outer conftest — which
pytest hits first — solves the ordering issue.

When the real ``homeassistant`` package is installed (e.g. inside a full HA
dev environment) the stub is skipped.
"""
...
# Make sure the HA config directory (parent of ``custom_components/``) is on
# the path so ``import custom_components.labeled_features`` works when tests
# are run from anywhere.
_HA_CONFIG_DIR = Path(__file__).resolve().parents[1]
...
def _install_homeassistant_stub() -> None:
    ...
    # Also stub voluptuous if missing (config_flow imports it at load time).
    try:
