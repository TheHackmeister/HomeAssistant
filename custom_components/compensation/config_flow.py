...
class CompensationConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_configure(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_ENTITY_ID, default=f"{ user_input[CONF_TRACKED_ENTITY_ID] }_calibrated"): cv.string,
                    vol.Optional(CONF_NAME, default=f"{ self.hass.states.get(user_input[CONF_TRACKED_ENTITY_ID]).name } Calibrated"): cv.string,
                    vol.Optional(CONF_ATTRIBUTE): vol.In( list(self.hass.states.get(user_input[CONF_TRACKED_ENTITY_ID]).attributes.keys()) ),
                    vol.Optional(CONF_PRECISION, default=DEFAULT_PRECISION): cv.positive_int,
                    vol.Optional(CONF_DEGREE, default=DEFAULT_DEGREE): vol.All(
                        vol.Coerce(int),
                        vol.Range(min=1, max=7),
                    ),
                    vol.Optional(CONF_UNIT_OF_MEASUREMENT, default=f"{ self.hass.states.get(user_input[CONF_TRACKED_ENTITY_ID]).attributes.get(CONF_UNIT_OF_MEASUREMENT) }"): cv.string,
                }
            ),
            errors=self._errors,
        )
